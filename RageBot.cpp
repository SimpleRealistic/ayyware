		...
		else
		{
			if ( ragebotconfig.bHitchance && hitchance( ) >= ragebotconfig.flHitchanceAmt * 1.5 || !ragebotconfig.bHitchance )
			{
				if ( AimAtPoint( m_local, point, pCmd, bSendPacket, pTarget ) )
				{
					...
					if ( ragebotconfig.bAutoFire || ragebotconfig.bAutoFire && ragebotconfig.iAutoFireMode == 1 && ragebotconfig.iAutoFireKey >= 0 && GUI.GetKeyState( ragebotconfig.iAutoFireKey ) )
					{
						pCmd->buttons |= IN_ATTACK;
...
// steamcommunity.com/id/simplerealistic - the lives he changed