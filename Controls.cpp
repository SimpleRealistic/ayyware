...
void CCheckBox::Draw(bool hover)
{
	...
	if (Checked)
	{
		draw.gradient_verticle(a.x, a.y, 9, 9, Color(167, 244, 66, 255), Color(12, 12, 12, 255));
		draw.outline(a.x, a.y, 9, 9, Color(2, 2, 2, 255));
	}
}
...
void CGroupBox::Draw(bool hover)
{
	...
	if (group_tabs.size())
	{
		...
		for (int i = 0; i < group_tabs.size(); i++)
		{
			...
			{
				draw.text(text_position[0] - (text_size.right / 2), text_position[1], group_tabs[i].name.c_str(), draw.fonts.menu, Color(140, 140, 140, 255));
			}
...
void CSlider::Draw(bool hover)
{
	...
	draw.gradient_verticle(a.x, a.y, Location, 7, Color(167, 244, 66, 255), Color(12, 12, 12, 255));
...
void CSlider2::Draw(bool hover)
{
	...
	float red = Menu::Window.VisualsTab.player.GlowR.GetValue();
	float green = Menu::Window.VisualsTab.player.GlowG.GetValue();
	float blue = Menu::Window.VisualsTab.player.GlowB.GetValue();
	float alpha = Menu::Window.VisualsTab.player.GlowOpacity.GetValue();



	draw.gradient_verticle(a.x, a.y, Location, 7, Color(red, green, blue, alpha), Color(12, 12, 12, 255));
	//draw.gradient_verticle(a.x, a.y, Location, 7, Color(255, 255, 255, 0), Color(15, 15, 15, 1));
	...
	RECT txtSize = draw.get_text_size(buffer, draw.fonts.menu_bold);
	//draw.text(a.x + Location - (txtSize.right / 2), a.y + 7 - (txtSize.bottom / 2), buffer, draw.fonts.menu_bold, Color(180, 180, 180, 255));
}
...
void CSlider3::Draw(bool hover)
{
	...
	float red = Menu::Window.VisualsTab.player.ChamR.GetValue();
	float green = Menu::Window.VisualsTab.player.ChamG.GetValue();
	float blue = Menu::Window.VisualsTab.player.ChamB.GetValue();
	//float alpha = Menu::Window.VisualsTab.player.GlowOpacity.GetValue();



	draw.gradient_verticle(a.x, a.y, Location, 7, Color(red, green, blue, 255), Color(12, 12, 12, 255));
...
void CSlider4::Draw(bool hover)
{
	...
	float red = Menu::Window.VisualsTab.player.ChamwallR.GetValue();
	float green = Menu::Window.VisualsTab.player.ChamwallG.GetValue();
	float blue = Menu::Window.VisualsTab.player.ChamwallB.GetValue();
	//float alpha = Menu::Window.VisualsTab.player.GlowOpacity.GetValue();
...
void CKeyBind::Draw(bool hover)
{
	...
	{
		draw.text(a.x, a.y, "[-]", draw.fonts.menu, Color(110, 110, 110, 255));
	}
	else
	{
		if (key == -1)
...
void CKeyBind::OnUpdate() {
	...
	RECT text_area;
	if (key == -1)
		text_area = draw.get_text_size("[-]", draw.fonts.menu);
	else
...
void CButton::Draw(bool hover)
{
	...
	if (hover)
	{
		draw.gradient_verticle(a.x, a.y, m_iWidth, m_iHeight, Color(25, 25, 25, 255), Color(35, 35, 35, 255));
		draw.text(TextX, TextY, Text.c_str(), draw.fonts.menu_bold, Color(167, 244, 66, 255));
	}
	else
	{
		draw.gradient_verticle(a.x, a.y, m_iWidth, m_iHeight, Color(35, 35, 35, 255), Color(25, 25, 25, 255));
	...
	draw.outline(a.x + 1, a.y + 1, m_iWidth - 2, m_iHeight - 2, Color(50, 50, 50, 255));
	...
	if (Items.size() > 0)
	{
		...
		if (IsOpen)
		{
			...
			for (int i = 0; i < Items.size(); i++)
			{
				...
				if (i == SelectedIndex)
					draw.text(a.x + 10, a.y + 20 + (i * 20) + 10 - (control_textsize.bottom / 2), Items[i].c_str(), draw.fonts.menu, Color(167, 244, 66, 255));
				else
...
void CMultiBox::Draw(bool hover)
{
	...
	for (int i = 0; i < items.size(); i++)
	{
		...
		if (!item.bselected)
			...
			draw.text(item_x, item_y, item.text, draw.fonts.menu, Color(167, 244, 66, 255));
	}
}
