...
void CEsp::CVisualsPlayer::draw_box(IClientEntity* m_entity, Box box, Color color) {
	...
	if (visualconfig.player.bBox) {
		if (!visualconfig.player.iBoxType) {
		...
		else if (visualconfig.player.bBox) {
			...
			draw.rect(box.x, box.y, width_corner, 1, Color(21, 21, 21, alpha));
			draw.rect(box.x, box.y, 1, height_corner, Color(21, 21, 21, alpha));

			draw.rect((box.x + box.w) - width_corner, box.y, width_corner, 1, Color(21, 21, 21, alpha));
			draw.rect(box.x + box.w, box.y, 1, height_corner, Color(21, 21, 21, alpha));

			draw.rect(box.x, box.y + box.h - 3, width_corner, 1, Color(21, 21, 21, alpha));
			draw.rect(box.x, (box.y + box.h) - height_corner - 3, 1, height_corner, Color(21, 21, 21, alpha));

			draw.rect((box.x + box.w) - width_corner, box.y + box.h - 3, width_corner, 1, Color(21, 21, 21, alpha));
			draw.rect(box.x + box.w, (box.y + box.h) - height_corner - 3, 1, height_corner + 1, Color(21, 21, 21, alpha));
		}
	}
}
...
float red2 = Menu::Window.VisualsTab.player.GlowG.GetValue();
float green = Menu::Window.VisualsTab.player.GlowR.GetValue();
...
void CEsp::CGlow::paint()
{
	...
	for (auto i = 0; i < m_pGlowObjManager->size; i++) {
		...
		if (m_entity->GetClientClass()->m_ClassID == 35) {
			...
			float m_flRed = visualconfig.player.cGlowRed, m_flGreen = visualconfig.player.cGlowGreen, m_flBlue = visualconfig.player.cGlowBlue;
